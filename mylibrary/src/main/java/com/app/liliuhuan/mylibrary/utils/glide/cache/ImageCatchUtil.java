...
public class ImageCatchUtil {
    ...
    private final String ImageExternalCatchDir = CommonUtil.getAppContext().getExternalCacheDir()
            + File.separator + CustomGlideModule.GLIDE_CARCH_DIR;
    ...
    public void clearImageDiskCache() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(() -> {
                    Glide.get(CommonUtil.getAppContext()).clearDiskCache();
                }).start();
            } else {
                Glide.get(CommonUtil.getAppContext()).clearDiskCache();
            }
    ...
    public void clearImageMemoryCache() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(CommonUtil.getAppContext()).clearMemory();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String getCacheSize() {
        try {
            ...
            return getFormatSize(getFolderSize(new File(CommonUtil.getAppContext().getCacheDir()
                    + File.separator + CustomGlideModule.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
        }
