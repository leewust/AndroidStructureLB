...
package com.app.liliuhuan.normallibrary.utils.rsa;
...
public class RSACipherStrategy extends CipherStrategy {
	...
	@Override
    public String decryptStr(String encryptContent, String charsetName) {
		if (mPrivateKey == null) {
			throw new NullPointerException("PrivateKey is null, please init it first");
		}
		String string = "";
	...
	@Override
    public byte[] decryptByte(String encryptContent) {
		if (mPrivateKey == null) {
			throw new NullPointerException("PrivateKey is null, please init it first");
		}
		byte[] encryptByte = decodeConvert(encryptContent);
