package com.app.liliuhuan.normallibrary.views.xrecyclerview;
...
public class XRecyclerView extends RecyclerView {
    ...
    private void init(Context context) {
        ...
        LoadingFooter footView = new LoadingFooter(context);
    ...
    private void loadMoreComplete() {
        ...
        if (previousTotal <= getLayoutManager().getItemCount()) {
            if (footView instanceof LoadingFooter) {
                ((LoadingFooter) footView).setState(LoadingFooter.STATE_COMPLETE);
            } else {
                footView.setVisibility(View.GONE);
            }
        } else {
            if (footView instanceof LoadingFooter) {
                ((LoadingFooter) footView).setState(LoadingFooter.STATE_NOMORE);
            } else {
                footView.setVisibility(View.GONE);
            }
            isnomore = true;
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                ...
                isLoadingData = true;
                if (footView instanceof LoadingFooter) {
                    ((LoadingFooter) footView).setState(LoadingFooter.STATE_LOADING);
                } else {
                    footView.setVisibility(View.VISIBLE);
                }
    ...
    public void setLoadMoreGone() {
        ...
        View footView = mFootViews.get(0);
        if (footView != null && footView instanceof LoadingFooter) {
            mFootViews.remove(0);
        }
    }
